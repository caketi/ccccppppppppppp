...
RandomListNode *copyRandomList(RandomListNode *head) {
  ...
  while (p) { // copy each node ----- 1->2->3  ----  1->1->2->2->3->3
...
RandomListNode *copyRandomList_v2(RandomListNode *head) {
  ...
  while (ptr) {
    ...
    if (ptr->random) { // node_map确认地址->节点的编号（原链表random指针指向的位置）
...
Node *reverseBetween_1(Node *head, int m, int n) {
  ...
  if (pre_head) { // 如果pre_head不空，说明不是从第一个节点开始翻转， m>1
...
Node *getIntersectionNode_v2(Node *headA, Node *headB) {
  ...
  if (list_A_len > list_B_len) { // 长链表移动head到和短链表的head一致
...
// 剑指offerII 029.排序的循环链表
Node *insertCir(Node *head, int insertVal) {
  Node *p = head, *max_node;
  while (p) {
    if (p->data > p->next->data)
      max_node = p;
    if (p->data <= insertVal && p->next->data >= insertVal)
      break;
    if (head == p->next) {
      p = max_node;
      break;
