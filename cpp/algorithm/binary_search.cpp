...
using namespace std;
// 1004. 最大连续1的个数III 给定一个0 1 组成的数组，最多可以将k个值从0变1，返回包含1的最长子数组的长度（仅仅）
// a= [1,1,1,0,1,0, 0] k=2  [1,1,1,0,1,0]
...
// 当前位置之前1的个数 n1 + n0 当前位置之后0的个数
// 926. 将字符串翻转到单调递增
int minFlipMonoIncr(string s) {
  int ans, n0 = 0, n1 = 0;
  // 先记录序列中所有0的数量，再把当前位置(单调点)之前的0删掉
  for (auto x : s)
    if (x == '0')
      n0 += 1;
  ans = n0;
...
int searchInsert(vector<int> &nums, int target) {
  ...
  while (index == -1) {
    ...
    if (target == nums[mid]) {
      index = mid;
    } else if (target < nums[mid]) { //  mid-1 < target < mid    mid
      if (mid == 0 || target > nums[mid - 1]) {
        index = mid;
      }
      end = mid - 1;
    } else if (target > nums[mid]) { // mid < target < mid+1
...
vector<int> searchRange(vector<int> &nums, int target) {
  ...
  ret[1] = binary_se_01(nums, target + 1) -
           1; // 最后一个位置， 8--ind--- 要找的7的最后位置ind-1
  return ret;
}
...
int findRadius(vector<int> &houses, vector<int> &heaters) {
  ...
  for (auto x : houses) {
    ...
    int b = (j ? x - heaters[j - 1]
               : a + 1); // a+1 --- 当j<=0时，a+1能让b在min(a,b)中返回小的a
    ans = max(ans, min(a, b));
  }
  return ans;
}
