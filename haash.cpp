...
class Nd {
public:
  Nd(int key = 0, int value = 0, Nd *next = nullptr)
...
public:
  ...
  int get(int key) { return h.get(key); }
  void put(int key, int value) {
    h.put(key, value);
    return;
  }
...
string decode(string shortUrl) { return h[shortUrl]; }
// len = 10 , 在s中出现次数超过一次
vector<string> findRepeatedDnaSequences(string s) {
  unordered_map<string, int> h;
  for (int i = 0, I = s.size() - 9; i < I; i++) { // 要求字串长度10
    h[s.substr(i, 10)] += 1;
  }
  vector<string> ret;
  for (auto x : h) {
...
TreeNode *getResult(TreeNode *root, TreeNode *target, int &k,
                    vector<int> &ret) {
  if (root == nullptr)
    return nullptr;
  if (root == target) {
