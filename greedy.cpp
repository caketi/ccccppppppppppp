...
int get_minimum_stop(
    int L, int P,
    vector<pair<int, int>>
        &stop) { // L -起点到终点的距离  P-起点初始的汽油量 ， stop-停靠点
  priority_queue<int> Q; // 存储油量的最大堆
  int result = 0;        // 记录加过几次油
  stop.push_back(make_pair(0, 0));
  sort(stop.begin(), stop.end(), CMP);
  for (int i = 0; i < stop.size();
       i++) { // 油够，往前冲，不够找法宝，够就冲，不够就加，加不够gg
...
bool canJump(vector<int> &nums) {
  ...
  while (jump < index.size() &&
         jump <=
             max_index) { // jump跳至数组尾部或jump超越了当前可以跳的最远位置
...
string removeKdigits(string num, int k) {
  ...
  for (int i = 0; i < num.length(); i++) {
    int number = num[i] - '0';
    while (S.size() != 0 && S[S.size() - 1] > number &&
           k > 0) { // 栈顶元素大于待选元素，谁大谁走
      S.pop_back();
      k--;
    }
  ...
  for (int i = 0; i < S.size(); i++) {
    result.append(
        1, '0' + S[i]); // Append multiple characters. 参数: __n – The number of
                        // characters to append. __c – The character to use.//
                        // 返回:Reference to this string. Appends __n copies of
                        // __c to this string.
  }
  if (result == "") {
    result = "0";
