...
struct node
{
    node(int key = 0, int h = 0, node *left = NIL, node *right = NIL) : key(key), h(h), left(left), right(right) {}
    int key;
    int h;
    node *left, *right;
    static node __NIL;
...
//             r = ind;
//             if(n->left) q.push(PNI(n->left, (ind-l) * 2)); // ind超界。。。 ind-l
...
//     int pos == 0;
//     while(inorder[pos] != preorder[0]) ++pos; //找界限
...
// 二叉树层序遍历 自上而下。自下而上.锯齿遍历（s形）（递归、队列）       k : 第几个数组 0 1 2 3---层数  父节点：上一层，子节点：下一层
...
class treetoDobuleList
{
    struct Node
    {
        ...
        Node *left;
        Node *right;
        Node() : val(0), left(left), right(right) {}
